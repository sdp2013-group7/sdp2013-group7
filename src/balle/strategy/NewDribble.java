...
public class NewDribble extends AbstractPlanner {
    ...
    private static Logger LOG = Logger.getLogger(NewDribble.class);

	private int currentSpeed = INITIAL_CURRENT_SPEED;
	private int turnSpeed = INITIAL_TURN_SPEED;
    private long lastNewDribbled = 0;
    private long firstNewDribbled = 0;
	private double MAX_NewDribble_PAUSE = 700; // ms
    private double MAX_NewDribble_LENGTH = 500; // ms
    ...
    public NewDribble(boolean triggerHappy) {
        super();
    ...
    public boolean shouldStopDribblingDueToNewDribbleLength() {
        double deltaStart = (System.currentTimeMillis() - firstNewDribbled);
        return deltaStart > MAX_NewDribble_LENGTH;
    }

    public boolean isInactiveForAWhile() {
        double deltaPause = (System.currentTimeMillis() - lastNewDribbled);

        return deltaPause > MAX_NewDribble_PAUSE;
    }
    public boolean isDribbling() {
        return !isInactiveForAWhile()
                && (!shouldStopDribblingDueToNewDribbleLength());
    }
	...
	public void onStep(Controller controller, Snapshot snapshot) throws ConfusedException {
        ...
        if (!isDribbling()) {
            ...
            firstNewDribbled = currentTime;
        }

        lastNewDribbled = currentTime;
...
*/
		if (isLeftGoal) {
	if ((!facingGoal) && (angle < Math.PI - threshold)) {
				controller.setWheelSpeeds(currentSpeed, currentSpeed
                        + turnSpeedToUse);
        ...
        if (facingGoal || (isTriggerHappy() && nearWall && !facingOwnGoalSide)
        	...
        	LOG.info("Dribblers off and kicking");
