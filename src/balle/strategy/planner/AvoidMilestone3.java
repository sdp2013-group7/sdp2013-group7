...
public class AvoidMilestone3 extends AbstractPlanner {
	...
	// Rotation data
	private boolean orientationBeforeSmaller;
	...
	protected void rotating(Controller controller) {
		...
		if (rotatingToAvoid) {
			...
			boolean c1 = orientationBeforeSmaller && ourRobot.getOrientation().degrees() < originalDirection.degrees();
			boolean c2 = !orientationBeforeSmaller && ourRobot.getOrientation().degrees() > originalDirection.degrees();
			LOG.info(ourRobot.getOrientation().degrees());
			LOG.info(originalDirection.degrees());
	...
	protected void moving(Controller controller) {
		...
		if (closeToWall(frontRect, pitchSides))
			...
			if (currentPosition.dist(startPoint) > 0.30) {
				...
				orientationBeforeSmaller = ourRobot.getOrientation().degrees() > originalDirection.degrees();
